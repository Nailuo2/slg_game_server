...
import (
	...
	"SLG_sg_server/server/game/gameConfig/general"
...
func (a *armyService) GetArmyByCityAndOrder(rid, cId int, order int8) (*data.Army, bool) {
	army := &data.Army{}
	ok, err := db.Engine.Table(army).Where("rid=? and cityId=? and order=?", rid, cId, order).Get(army)
	if err != nil {
		log.Println("军队查询出错", err)
		return nil, false
	}
...
func (a *armyService) GetArmy(cid int, order int8) *data.Army {
	army := &data.Army{}
	ok, err := db.Engine.Table(army).Where("cityId=? and order=?", cid, order).Get(army)
	if err != nil {
